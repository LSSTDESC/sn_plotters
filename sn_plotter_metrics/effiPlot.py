...
class plotEffi:
    ...
    def plotEffi(self):
        ...
        for healpixID in np.unique(self.data['healpixID']):
            ...
            for season in np.unique(sel['season']):
                ...
                self.plotEffi_indiv(sel,ax,healpixID,season,'effi', 'effi_err','Observing Efficiencies', ls='-',label='season {}'.format(season))
    ...
    def plotCumul(self,data, ax,healpixID,season,sn_x1=-2.0,sn_color=0.2,zlim_coeff=0.95,label=''):
        ...
        nsn_cum_norm, zlimit = self.get_zlims(data, ax,healpixID,season,sn_x1=sn_x1,sn_color=sn_color,zlim_coeff=zlim_coeff,zplot=zplot)
    ...
    def plotNSN(self,data, ax,healpixID,season,sn_x1=0.0,sn_color=0.0,zlim_coeff=0.95,label=''):
        ...
        nsn_cum_norm, zlimit = self.get_zlims(data, ax,healpixID,season,sn_x1=sn_x1,sn_color=sn_color,zlim_coeff=zlim_coeff,zplot=zplot)
        zplotb = np.arange(0.01,zlimit,0.01)
        nsn_zlimit = np.cumsum(effiInterp(zplotb)*rateInterp(zplotb))[-1]
        print(nsn_zlimit)
        zcomp = '$N_{SN}$ = '
        ax.text(0.05,12-0.8*season,
